...
impl TaskManager {
    ...
    fn get_current_start_time_ms(&self) -> usize {
        let inner = self.inner.exclusive_access();
        inner.tasks[inner.current_task].start_times
    }

    fn get_current_syscall_times(&self) -> [u32;MAX_SYSCALL_NUM] {
        let inner = self.inner.exclusive_access();
        let current = inner.current_task;
        // get info from syscal_times
        let mut system_times = [0u32;MAX_SYSCALL_NUM];
        for (syscall, num)  in &inner.tasks[current].syscall_times {
            system_times[*syscall] = *num;
        }
        system_times
    }
    ...
    fn update_current_syscall_times(&self, syscall_id: usize) {
        let mut inner = self.inner.exclusive_access();
        let current = inner.current_task;
        let mut syscall_times = inner.tasks[current].syscall_times.entry(syscall_id).or_insert(0);
        *syscall_times += 1;
    }
...
pub fn current_start_time_ms() -> usize {
